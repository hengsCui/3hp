...
# ================= Gemini Global Patch =================
import biosteam as bst
# 强行定义全局 feedstock，防止 lambda 函数报错
if 'feedstock' not in globals():
    feedstock = bst.Stream('feedstock_global_patch', Water=100)
    feedstock.F_mass = 1000 # 给个初始值防止除以零
    feedstock.price = 0.28 # 给个价格
# =======================================================
...
# Gemini Deleted Garbage Line 139
    if 'feedstock' not in locals(): feedstock = bst.Stream('feedstock_repaired'); feedstock.imass['Water'] = 1; feedstock.imass['Water'] = 1 # Gemini_Fix: 修复缺失变量
    # Gemini Fix: 强制连接 U101 防止 NoneType 报错
    if 'U101' not in locals(): U101 = bst.Storage('U101_Gemini_Patch')
    feedstock_sink = U101
    ...
    #%%
    s = flowsheet.stream
    create_facilities(feedstock=feedstock, 
...
# Gemini Deleted Garbage Line 442
AA = s.AcrylicAcid
...
spec = ProcessSpecification(feedstock=feedstock, 
    evaporator = u.F301,
    pump = None,
    mixer = u.M304,
